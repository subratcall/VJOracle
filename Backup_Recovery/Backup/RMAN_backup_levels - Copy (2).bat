...
set RMANLEVEL=full
del %RMAN_DEST%*.* /q
echo backup full filesperset 4 format '%RMAN_DEST%\FULL_%ORACLE_SID%_t%%t_s%%s_p%%p.db' database tag='full_backup';>> %rcvfile%
...
:INCR
set RMANLEVEL=incr
echo backup incremental level %LEVELBCK% filesperset 4 format '%RMAN_DEST%\INCR_%ORACLE_SID%_t%%t_s%%s_p%%p.db' database tag='incremental';>> %rcvfile%
rem rman target / catalog=%CATALOG_PASS% cmdfile=%rcvfile% msglog=%LOG_DEST%rman_%ORACLE_SID%.log
rem rman target / nocatalog cmdfile=%rcvfile% msglog=%LOG_DEST%\rman_%ORACLE_SID%.log
rem del %rcvfile%
goto END
...
:CUMU
set RMANLEVEL=cumu
echo backup incremental level %LEVELBCK% CUMULATIVE filesperset 4 format '%RMAN_DEST%\CUMU_%ORACLE_SID%_t%%t_s%%s_p%%p.db' database tag='cumulative';>> %rcvfile%
rem rman target / catalog=%CATALOG_PASS% cmdfile=%rcvfile% msglog=%LOG_DEST%rman_%ORACLE_SID%.log
rem rman target / nocatalog cmdfile=%rcvfile% msglog=%LOG_DEST%\rman_%ORACLE_SID%.log
rem del %rcvfile%
...
rem echo backup archivelog all filesperset 30 format '%RMAN_DEST%\ARC_%ORACLE_SID%_%%T_s%%s_p%%p.arc' delete input ;>> %rcvfile%
echo backup archivelog all filesperset 30 format '%RMAN_DEST%\ARC_%ORACLE_SID%_%%T_s%%s_p%%p.arc' ;>> %rcvfile%
echo backup current controlfile format '%RMAN_DEST%\CTL_%ORACLE_SID%_%%T_s%%s_p%%p.ctl';>> %rcvfile%
...
%ORACLE_HOME%\bin\rman target / nocatalog cmdfile=%rcvfile% msglog=%LOG_DEST%\%ORACLE_SID%_%BKTYPE%_%RMANLEVEL%_%M%%D%%Y%.log
