...
rem     E:\db_backup\RMAN_backup_levels.bat vj12c hot 0   -- Full backup (incr 0)
rem     E:\db_backup\RMAN_backup_levels.bat vj12c arch -- Archivelog backup
rem 	bktype 
...
set ORACLE_HOME=C:\oracle\ora12102
SET ORACLE_SID=%1%
SET BKTYPE=%2%
rem SET RMAN_DB=%2%
...
SET RMAN_DEST=E:\db_backup\%ORACLE_SID%\rman
SET LOG_DEST=E:\db_backup\%ORACLE_SID%\rman\log
...
echo backup archivelog all filesperset 30 format '%RMAN_DEST%\arch\ARC_%ORACLE_SID%_%%T_s%%s_p%%p.arc' delete input ;>> %rcvfile%
rem echo backup as compressed backupset archivelog all filesperset 30 format '%RMAN_DEST%\arch\ARC_%ORACLE_SID%_%%T_s%%s_p%%p.arc' ;>> %rcvfile%
